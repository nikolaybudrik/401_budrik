...
namespace YOLOv4MLNet.DataStructures
{
    ...
    {
        static string PathTmp = Path.GetTempPath();
        ...
        public List<YoloV4Result> GetResults(string[] categories, Bitmap bitmap, Lol.MainWindow mainWindow, float scoreThres = 0.5f, float iouThres = 0.5f)
        {
            ...
            using (var g = Graphics.FromImage(bitmap))
            {
                ...
                while (f < postProcesssedResults.Count)
                {
                    ...
                    var tmp = new YoloV4Result();
                    tmp.BBox0 = res.Take(4).ToArray()[0];
                    tmp.BBox1 = res.Take(4).ToArray()[1];
                    tmp.BBox2 = res.Take(4).ToArray()[2];
                    tmp.BBox3 = res.Take(4).ToArray()[3];
                    tmp.Label = label;
                    tmp.Confidence = conf;
                    ...
                    g.DrawString(tmp.Label + " " + tmp.Confidence.ToString("0.00"),
                                 new Font("Arial", 12), Brushes.Blue, new PointF(x1, y1));

                    string pathtmp = Path.Combine(PathTmp, @"tmp.jpg" + f.ToString());
                    bitmap.Save(pathtmp);
                    mainWindow.Change_Image(pathtmp);
